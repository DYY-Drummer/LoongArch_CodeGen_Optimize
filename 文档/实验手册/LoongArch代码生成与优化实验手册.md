...
​		上图来自于书籍《Computer Architecture A Quantitative Approach Fourth Edition》，这种指令流水线设计旨在让处理器每个时钟周期都能执行一条指令。非流水线设计的机器需要等待一条指令的五个阶段全部结束后才会获取下一条指令，每五个时钟周期才能完成一条指令；而流水线设计的机器就好像为机器指令的五个阶段分别设置了一台车床，同一时刻内可以有五条指令分别位于不同的车床上，当上一条指令进入解码阶段，就获取下一条指令，这样能够使得每个时钟周期都能完成一条指令，指令执行效率是非流水线设计的五倍。
...
​		如1.1.4节中介绍的LoongArch指令运行的5阶段流水线设计：指令获取、指令解码、执行、内存访问和写回，对于跳转指令，显然在获取它之前，指令中的跳转地址已经根据重定位信息计算出了具体的值回填到了指令里。如下图所示，以B指令为例，B指令中的$BB0_2在重定位之后会被解析为该相对PC的偏移地址，即(X+16) - X = 16字节。此时问题就来了，程序计数器PC在“指令获取”之后就会将PC+4（字节）以指向下一条指令，此时PC=X+4，当B指令执行时，PC=PC+16=X+20，指向的是BB0_2块的第二条指令nop而不是第一条指令。为了修正这一偏差，我们需要在adjustFixupValue()方法中手动将分支跳转指令的重定位地址值-4，即真正的跳转偏移值应该为“目标指令 - 当前指令的下一条指令”。
...
​		许多基本块的最后一条指令是一条无条件跳转指令B，且跳转的目标是紧接着的下一个基本块，或者说下一条指令。根据7.1节中的讨论，此时B指令的跳转偏移值应该为目标指令的地址减去下一条指令的地址，相当于“`b 0`"，显然，这条跳转指令是无用的。本节将会添加一个优化Pass以移除冗余的无条件跳转指令。
...
​		在addPreEmitPass()中注册优化Pass。

+ **LoongArchDelUselessB.cpp**

​		检查每个BasicBlock的最后一条指令，若为B指令且跳转目标为下一个BasicBlock，则删除该指令。为了保证该优化在每次进入新的Function时都会执行一次，需要将其重写在runOnMachineFunction()方法中。同理，重写runOnMachineBasicBlock()以使得优化在每次进入新的BasicBlock都会执行。

​		使用LLVM内置的STATISTIC()方法添加自定义统计数据NumDelB，用于输出删除的B指令的数量，该数据在执行删除操作时手动计算。

​		添加命令行选项`enable-loongarch-del-useless-b`用以控制是否启用冗余无条件跳转删除优化，由于该优化和其它优化间不存在冲突，所以默认启用，如果需要禁用需要在llc命令中将该选项指定为false。

+ **编译测试**

​		使用7-3/test.c文件进行测试，加上"-stats"指令以在终端输出统计信息，查看删除的冗余分支跳转指令数量：`./llc -march=loongarch -mcpu=loongarch32 -relocation-model=static -filetype=asm -stats test.bc -o test_optimized.s`。在编译选项中加上`enable-loongarch-del-useless-b=false`来查看未执行优化的汇编。
...
===-------------------------------------------------------------------------===
...
 4 del-b        - Number of useless b deleted
...
```

​		可以看到一共删除了4个冗余的跳转指令，且删除后指令连续的两个基本块合并为了一个基本块，基本块标签序号也重新进行了排序。

​		不过此优化Pass还有待完善，对于只有一条指令且是冗余无条件跳转指令的基本块（如图中BB0_6），将该冗余指令删除后，BB0_6和BB0_7表示同一地址，该基本块的标签成为冗余，但又无法简单地直接将其删除，因为其它地方存在引用了BB0_6（重新排序后的BB0_4）的跳转指令。如何将删除冗余无条件跳转指令后的冗余标签的引用替换为下一个标签的引用，并最终删除冗余标签的逻辑本文尚未实现，或许应该作为另一个优化Pass单独执行。



## 7.4 Select指令

​		当clang执行前端编译时使用O1及以上级别的优化时，LLVM IR会将只有一条赋值指令的迷你分支基本块（例如C语言中的三目运算符“`cond ? m:n`"语句）使用select指令来处理，而不是为其创建条件分支，如下图所示。select指令有三个输入一个输出，第一个输入为判断值，第二、三个输入为备选值，若判断值为真，则将第一个备选值赋值给输出，否则将第二个备选值赋值给输出。当使用O0级别的优化时，只有三目运算符`?:`才会被转换为select，而迷你的if-else仍会生成单独的BasicBlock，使用条件分支处理。
...
​			对于流水线指令架构的机器，前端会尽可能地使用select指令替代条件分支指令可以有效地减少因分支跳转而造成的流水线停滞损耗。这种替换可以在前端clang编译时使用`-O0`选项来禁止。本节将实现对IR select指令的支持。

​		

+ **LoongArchInstrInfo.td**

​		LoongArch指令集中没有与select相同功能的指令，需要用MASKEQZ、MASKNEZ和OR指令组合实现。MASKEQZ、MASKNEZ均为3R类型的指令，三个寄存器分别为输出寄存器rd，备选值寄存器rj和判断值寄存器rk。两个指令根据rk是否等于0选择将rj赋值给rd或将rd置0。匹配模式为：

`Pat<(select GPR:$cond, GPR:$t, GPR:$f),
          (OR (MASKEQZ GPR:$t, GPR:$cond), (MASKNEZ GPR:$f, GPR:$cond))>`

+ **LoongArchISelLowering(.h/.cpp)**

​		Select在后端DAG优化阶段会和SetCC指令合并为select_cc节点，需要设置setOperationAction()方法的Expand模式阻止其合并，这样就不用再额外处理select_cc了。
...
​		实现了两种参数传递的约束：一种基于ILP32S的，使用8个整数参数寄存器以及栈传参，最多16个参数的约束CC_LoongArchILP32S；一个用于调试后端的，只使用栈来传递参数的CC_LoongArch_Stack_only。CC_LoongArchILP32S允许将64位整数和浮点数格式的参数存放在参数寄存器中，但是不能将64位参数的上半部分和下半部分分开存在寄存器和栈中，所以需要保证至少有两个参数寄存器可供分配，即当可分配的参数寄存器只剩A7时（参数寄存器是顺序分配的），放弃A7，将64位参数和剩余参数全部存放在栈中。如果不放弃A7，而是将其分配给后面的32位参数，将会打乱默认的参数存储顺序，需要额外的指令逻辑维护这个特别的参数地址对应关系，得不偿失。
...
​		本节使用“函数唤起”的名字以区别于“函数调用”，是指代整个函数调用过程中“索引目标函数地址并进入子程序”的这一步。本节将实现函数唤起指令的匹配模式和目标函数地址操作数的解析方法，使后端能够成功唤起函数。
