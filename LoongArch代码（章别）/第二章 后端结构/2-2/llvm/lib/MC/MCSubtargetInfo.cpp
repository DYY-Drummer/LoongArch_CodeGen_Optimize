...
static void ApplyFeatureFlag(FeatureBitset &Bits, StringRef Feature,
                             ArrayRef<SubtargetFeatureKV> FeatureTable) {
    std::cout<<"Feature in ApplyFeatureFlag before force assignment:"<<Feature.str()<<std::endl;
    Feature=StringRef("+loongarch32");
  ...
  for (const std::string &Feature : Features.getFeatures()) {
    ...
    if (Feature == "+help")
      ...
      cpuHelp(ProcDesc);
        ...
        std::cout<<"ApplyFeatureFlag in getFeatures is used :"<<Feature<<std::endl;
    }
...
bool LoongArchDisableUnreconginizedMessage = false;
void MCSubtargetInfo::InitMCProcessorInfo(StringRef CPU, StringRef FS) {
  FeatureBits = getFeatures(CPU, FS, ProcDesc, ProcFeatures);
    #if 1 // Disable reconginized processor message. For loongarch
    if (TargetTriple.getArch() == llvm::Triple::loongarch)
        LoongArchDisableUnreconginizedMessage = true;
    #endif
  if (!CPU.empty())
...
void MCSubtargetInfo::setDefaultFeatures(StringRef CPU, StringRef FS) {
  FeatureBits = getFeatures(CPU, StringRef("+loongarch32"), ProcDesc, ProcFeatures);
}
