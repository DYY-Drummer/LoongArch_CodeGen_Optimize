...
// LoongArch ILP32S ABI rules:
// ---
//===----------------------------------------------------------------------===//

// For debug purpose : passed in stack only.
static bool CC_LoongArch_Stack_only(unsigned ValNo, MVT ValVT, MVT LocVT,
...
// ILP32S Calling convention : Passed first 8 i32 arguments in general registers
// and others in stack.
static bool CC_LoongArchILP32S(unsigned ValNo, MVT ValVT, MVT LocVT,
                       CCValAssign::LocInfo LocInfo, ISD::ArgFlagsTy ArgFlags,
                       CCState &State) {
    static const MCPhysReg IntRegs[] = {
        LoongArch::A0, LoongArch::A1, LoongArch::A2, LoongArch::A3,
        LoongArch::A4, LoongArch::A5, LoongArch::A6, LoongArch::A7
    };
    ...
    // f32 and f64 are allocated in a0 - a7 when either of the following
    // is true: function is vararg, argument is 9rd or higher, there is previous
    ...
    if (ValVT == MVT::i32 || (ValVT == MVT::f32 && AllocateFloatsInIntReg)) {
        ...
        // the allocated register must be a0 - a6,
        // ensure that the higher and lower part of i64 arg are not separated in
        // register and stack respectively.
        if (isI64 && (Reg == LoongArch::A7))
            Reg = State.AllocateReg(IntRegs);
        LocVT = MVT::i32;
    } else if (ValVT == MVT::f64 && AllocateFloatsInIntReg) {
        // Allocate int register. If first
        // available register is LoongArch::A7, shadow it too.
        Reg = State.AllocateReg(IntRegs);
        if (Reg == LoongArch::A7)
            Reg = State.AllocateReg(IntRegs);
    ...
    // if no reg can be allocated for this ValVT, save it in the stack.
    if (!Reg) {
        unsigned Offset = State.AllocateStack(ValVT.getSizeInBits() >> 3,
                                              OrigAlign);
        State.addLoc(CCValAssign::getMem(ValNo, ValVT, Offset, LocVT, LocInfo));
    } else
    // if there is a reg can be allocated for this ValVT.
...
static const MCPhysReg ILP32SIntRegs[] = {
        LoongArch::A0, LoongArch::A1, LoongArch::A2, LoongArch::A3,
        LoongArch::A4, LoongArch::A5, LoongArch::A6, LoongArch::A7
};
...
LoongArchTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
    ...
    LoongArchMachineFunctionInfo *FuncInfo = MF.getInfo<LoongArchMachineFunctionInfo>();
    ...
    LoongArchCC::SpecialCallingConvType SpecialCallingConv =
            getSpecialCallingConv(Callee);
    LoongArchCC LoongArchCCInfo(CallConv, ABI.IsILP32S(),
                      CCInfo, SpecialCallingConv);

    LoongArchCCInfo.analyzeCallOperands(Outs, IsVarArg,
    ...
    if (IsTailCall)
        IsTailCall =
                isEligibleForTailCallOptimization(LoongArchCCInfo, NextStackOffset,
                                                  *MF.getInfo<LoongArchMachineFunctionInfo>());

    if (!IsTailCall && CLI.CS && CLI.CS.isMustTailCall())// Become "CallBase *CB" in LLVM 15
    ...
    SDValue StackPtr =
            DAG.getCopyFromReg(Chain, DL, LoongArch::SP,
                               getPointerTy(DAG.getDataLayout()));

    // With EABI is it possible to have 16 args on registers.
    std::deque< std::pair<unsigned, SDValue> > RegsToPass;
    SmallVector<SDValue, 8> MemOpChains;
    LoongArchCC::byval_iterator ByValArg = LoongArchCCInfo.byval_begin();
    ...
    for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
        ...
        if (Flags.isByVal()) {
            ...
            assert(ByValArg != LoongArchCCInfo.byval_end());
            assert(!IsTailCall &&
                   "Do not tail-call optimize if there is a byval argument.");
            passByValArg(Chain, DL, RegsToPass, MemOpChains, StackPtr, MFI, DAG, Arg,
                         LoongArchCCInfo, *ByValArg, Flags, Subtarget.isLittle());
            ++ByValArg;
            continue;
        }
        //@ByVal Arg }
        ...
        // Register can't achieve at this point
        assert(VA.isMemLoc());
    ...
    bool IsPICCall = IsPIC; // true if calls are translated to JARA $t7
    ...
    if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
        if (IsPICCall) {
            ...
            if (InternalLinkage)
                ...
                Callee = getAddrGlobal(G, Ty, DAG, LoongArch::MO_GOT_CALL, Chain,
                                       FuncInfo->callPtrInfo(Val));
        } else
            Callee = DAG.getTargetGlobalAddress(G->getGlobal(), DL,
                                                getPointerTy(DAG.getDataLayout()), 0,
                                                LoongArch::MO_NO_FLAG);
        ...
        if (!IsPIC) // static
            ...
            Callee = getAddrGlobal(S, Ty, DAG, LoongArch::MO_GOT_CALL, Chain,
...
SDValue
LoongArchTargetLowering::passArgOnStack(SDValue StackPtr, unsigned Offset,
...
getOpndList(SmallVectorImpl<SDValue> &Ops,
    ...
    // T7 should contain the address of the callee function if
    // -reloction-model=pic or it is an indirect call.
    if (IsPICCall || !GlobalOrExternal) {
        unsigned T7Reg = LoongArch::T7;
        RegsToPass.push_front(std::make_pair(T7Reg, Callee));
    } else
        Ops.push_back(Callee);

    // Insert node "GP copy globalreg" before call to function.
    //
    // R_LOONGARCH_CALL* operators (emitted when non-internal functions are called
    // in PIC mode) allow symbols to be resolved via lazy binding.
    // The lazy binding stub requires GP to point to the GOT.
    if (IsPICCall && !InternalLinkage) {
        unsigned GPReg = LoongArch::GP;
        EVT Ty = MVT::i32;
...
SDValue
LoongArchTargetLowering::LowerCallResult(SDValue Chain, SDValue InFlag,
    ...
    LoongArchCCInfo.analyzeCallResult(Ins, Subtarget.abiUsesSoftFloat(),
                                 CallNode, RetTy);
...
const {
    ...
    LoongArchCCInfo.analyzeFormalArguments(Ins, UseSoftFloat, FuncArg);
    ...
    for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
        ...
        if (Flags.isByVal()) {
            ...
            copyByValRegs(Chain, DL, OutChains, DAG, Flags, InVals, &*FuncArg,
                          LoongArchCCInfo, *ByValArg);
            ++ByValArg;
            continue;
        }
        //@byval pass }
        // Arguments stored on registers
        if (ABI.IsILP32S() && IsRegLoc) {
...
//@Ordinary struct type: 1 {
    for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
        ...
        // the sret argument into $a0 for the return. Save the argument into
...
LoongArchTargetLowering::LoongArchCC::SpecialCallingConvType
LoongArchTargetLowering::getSpecialCallingConv(SDValue Callee) const {
    LoongArchCC::SpecialCallingConvType SpecialCallingConv =
            LoongArchCC::NoSpecialCallingConv;
...
void LoongArchTargetLowering::LoongArchCC::handleByValArg(unsigned ValNo, MVT ValVT,
                                                MVT LocVT,
    ...
    unsigned Alignment = std::min(std::max(ArgFlags.getOrigAlign(), RegSize),
                               RegSize * 2);

    if (useRegsForByval())
        allocateRegs(ByVal, ByValSize, Alignment);
...
const ArrayRef<MCPhysReg> LoongArchTargetLowering::LoongArchCC::intArgRegs() const {
    return makeArrayRef(ILP32SIntRegs);
}

llvm::CCAssignFn *LoongArchTargetLowering::LoongArchCC::fixedArgFn() const {
    if (IsILP32S)
        return CC_LoongArchILP32S;
    else // Is Debug mode
        return CC_LoongArch_Stack_only;
}
unsigned LoongArchTargetLowering::LoongArchCC::reservedArgArea() const {
    return (IsILP32S && (CallConv != CallingConv::Fast)) ? 32 : 0;
}

void LoongArchTargetLowering::LoongArchCC::allocateRegs(ByValArgInfo &ByVal,
                                              unsigned ByValSize,
                                              unsigned Align) {
...
copyByValRegs(SDValue Chain, const SDLoc &DL, std::vector<SDValue> &OutChains,
              ...
              const LoongArchCC &CC, const ByValArgInfo &ByVal) const {
    MachineFunction &MF = DAG.getMachineFunction();
    MachineFrameInfo &MFI = MF.getFrameInfo();
...
passByValArg(SDValue Chain, const SDLoc &DL,
             ...
             const LoongArchCC &CC, const ByValArgInfo &ByVal,
