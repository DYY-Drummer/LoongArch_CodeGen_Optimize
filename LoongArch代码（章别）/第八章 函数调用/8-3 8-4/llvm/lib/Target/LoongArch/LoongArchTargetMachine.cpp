...
LoongArchTargetMachine::LoongArchTargetMachine(const Target &T, const Triple &TT,
        ...
        : LLVMTargetMachine(T, computeDataLayout(TT, CPU, Options, true), TT,
          ...
          isLittle(true), TLOF(std::make_unique<LoongArchTargetObjectFile>()),
          ABI(LoongArchABIInfo::computeTargetABI()),
          DefaultSubtarget(TT, CPU, FS, true, *this) {
    initAsmInfo();
}
...
void LoongArchPassConfig::addPreEmitPass() {
    LoongArchTargetMachine &TM = getLoongArchTargetMachine();

    addPass(createLoongArchDelBPass(TM));

    // Fixit: if uncomment this, runOnMachineFunction() in LoongArchLongBranch.cpp
    // will always emit $_gp_disp info whether we enable the long branch pass by
    // "force-loongarch-long-branch" option or not.
    //addPass(createLoongArchLongBranchPass(TM));
